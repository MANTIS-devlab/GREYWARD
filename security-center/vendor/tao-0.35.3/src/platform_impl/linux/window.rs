...
impl Window {
  pub(crate) fn new<T>(
    ...
    // GREYWARD hardwires compositor-owned Wayland decorations; labwc draws the native title bar.
    let greyward_labwc_server_decorations =
      std::env::var_os("GREYWARD_LABWC_SERVER_DECORATIONS").is_some();
    ...
    window.set_decorated(attributes.decorations && !greyward_labwc_server_decorations);
